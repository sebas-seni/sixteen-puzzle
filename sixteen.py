...
ITERACIONES_RANDOM = random.randint(5, 20)
...
def crear_tablero(n_filas: int, n_columnas: int) -> list[list[int]]:
    ...
    for _i in range(n_filas):
        fila = []
        for _j in range(n_columnas):
...
def mezclar_tablero(tablero: list[list[int]]):
    ...
    posibles_rotaciones = [rotar_arriba, rotar_abajo, rotar_izquierda, rotar_derecha]

    for _iteracion in range(ITERACIONES_RANDOM):
        rotacion_seleccionada = random.choice(posibles_rotaciones)

        if rotacion_seleccionada in (rotar_izquierda, rotar_derecha):
            fila_random = random.randint(0, len(tablero) - 1)
            rotacion_seleccionada(tablero, fila_random)
        else:
            columna_random = random.randint(0, len(tablero[0]) - 1)
            rotacion_seleccionada(tablero, columna_random)
